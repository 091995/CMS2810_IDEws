/*!
 ...
 \defgroup  SMCmdLogic Îáğàáîòêà êîìàíä è çàäàíèé, ïîñòóïàşùèõ èç ğàçëè÷íûõ èñòî÷íèêîâ. (ñì. TSM_CmdLogic)
 @{
*/
...
void SM_CmdLogicInit(TSM_CmdLogic *p) {
	...
	/* Ğàçğåøèòü íîãàì ğàáîòàòü íà âõîä, ıòî êíîïêè ñòàğò è ñòîï */
	/* Äëÿ âêëş÷åíèÿ/âûêëş÷åíèÿ äàííîãî êîíòğîëëåğà èñïîëüçîâàíû âíåøíèå äèñêğåòíûå âõîäû */

	/*
	 *		Çäåñü îïèñûâàşòñÿ (îòêëş÷àåòñÿ àëüòåğíàòèâíàÿ ôóíêöèÿ,
	 *		ğàçğåøàåòñÿ ğàáîòà íà âûõîä) íîæêè äëÿ ğàáîòû ñ êíîïêàìè
	 *		íà ñàìîì êîíòğîëëåğå. Íî òàê êàê äëÿ äàííîãî êîíòğîëëåğà
	 *		ìû èñïîëüçóåì âíåøíèå äèñêğåòíûå âõîäû (ñì. V_DIO), îñòàâèì
	 *		ıòî ìåñòî ïóñòûì.
	 */
#endif
}
	...
	case CMD_LOGIC_TURNED_OFF: {	//ñîñòîÿíèå 0- ÎÒÊËŞ×ÅÍ
		...
		if (sm_prot.state == PROT_ON_OK) {//åñëè ÄÀ çàùèò âêëş÷èëñÿ, îáğàáàòûâàåì êîìàíäû âêëş÷åíèÿ
			if ((cmd.bit.start == 1) && (sm_sys.state == SYS_READY)) {//åñòü áèòîâàÿ êîìàíäà ñòàğòà
				...
				else {//íàäî êîíòğîëèğîâàòü öåïü çàğÿäà
				    if (udControl.StateOn == 1) { //Åñëè ÇÏÒ çàğÿæåíî è ğåëå âêëş÷åíî - çàïóñêàåì
				        p->state = CMD_LOGIC_TURNED_ON;	//ïîåõàëè
				    }
				    else { //ïîïûòêà ïóñêà, à ğåëå íå âêëş÷åíî
				    	/*Çàùèòà îò ïóñêà ïğè íåçàøóíòèğîâàííîì ğåëå ïëàâíîãî çàğÿäà ÇÏÒ*/
