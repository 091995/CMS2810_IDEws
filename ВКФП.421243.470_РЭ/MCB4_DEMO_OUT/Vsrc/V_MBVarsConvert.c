...
void MBVarsConv_calc(TMBVarsConv *p) {
	...
	if (ModBus.Enabled == 1){ // Если включен MODBUS
		...
		for (i = 0; i < /*ModBus.*/MBInternals.NumOfHRs; i++)// Проверяем все элементы словаря HR (редактируемых параметров)
			if (mbodHR[i].type == 1){// Если параметр менялся, то анализируем, как его преобразовать

				mbodHR[i].type = 0;		// Сбрасываем флаг изменения параметра
				...
				switch (index){
					...
					case  2001:  // текущая частота вращения
						...
						number = fabs(number);
#ifdef CAN_1_ENABLE
						scale = (float) co1_vars.co_scaleNum14;	//напряжение ЗПТ базовое
#else
						scale = (float) co2_vars.co_scaleNum14;	//напряжение ЗПТ базовое
#endif
						scaled = number / scale;
...
#ifdef CAN_1_ENABLE
						scale = (float) co1_vars.co_scaleNum7;	//напряжение ЗПТ базовое
#else
...
#ifdef CAN_1_ENABLE
	scale = (float) co1_vars.co_scaleNum14;	// Частота вращения (об/мин)
#else
...
#ifdef CAN_1_ENABLE
	scale = (float) co1_vars.co_scaleNum4;	// Частота вращения (об/мин)
#else
	scale = (float) co2_vars.co_scaleNum4;	// Частота вращения (об/мин)
#endif
	scaled = scale * number;
	p->Vars.CurIs = scaled;
