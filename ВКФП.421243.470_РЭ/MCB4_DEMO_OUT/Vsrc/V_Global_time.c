...
void GlobalTime_read_PowerOn_time(TGlobalTime *p) { //чтение work_time
    spimem.spiaddr = GLOBAL_TIME_POWER_ON_TIME_ADDR;
    spimem.startaddr = (Uint16*) (&(p->PowerOn_time));
    spimem.data_length = 4;
...
void GlobalTime_read_oper_time(TGlobalTime *p) { //чтение operational_time
    spimem.spiaddr = GLOBAL_TIME_OPERATIONAL_TIME_ADDR;
    spimem.startaddr = (Uint16*) (&(p->operational_time));
    spimem.data_length = 4;
    spimem.readspi(&spimem);
}
