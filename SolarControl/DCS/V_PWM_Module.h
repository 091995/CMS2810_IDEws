...
struct SPWM_Module {
	...
	int32 gammaA_prev; //!< Скважность фазы А в размерности таймера ШИМ, для чтения.
	int32 gammaB_prev; //!< Скважность фазы B в размерности таймера ШИМ, для чтения.
	int32 gammaC_prev; //!< Скважность фазы C в размерности таймера ШИМ, для чтения.
	...
	Uint32 I_ref_pwm; //_IQ24 max=IQ(1);
	Uint32 U_ref_fan_pwm; //_IQ24 max = IQ(1);
	...
	void (*PDP_trip_reset)(volatile struct SPWM_Module*);
};
...
void PWM_Module_PDP_trip_reset(TPWM_Module *);
