...
//		IA_pu = -p->IA_int / (_IQ16mpy(adc.Imeas_a_gain, inv_params._1_I_nom));
//		adc.Imeas_a_offset  = IA_pu >> 1;
//
//		IB_pu = -p->IB_int / (_IQ16mpy(adc.Imeas_b_gain, inv_params._1_I_nom));
//		adc.Imeas_b_offset = IB_pu >> 1;
//
//		IC_pu = -p->IC_int / (_IQ16mpy(adc.Imeas_c_gain, inv_params._1_I_nom));
//		adc.Imeas_c_offset = IC_pu >> 1;
//	} else {//модуль выключен, инициализируем интеграторы текущими значенями смещений
//		p->IA_int=-(((long)adc.Imeas_a_offset)<<1)*_IQ16mpy(adc.Imeas_a_gain, inv_params._1_I_nom);
//		p->IB_int=-(((long)adc.Imeas_b_offset)<<1)*_IQ16mpy(adc.Imeas_b_gain, inv_params._1_I_nom);
//		p->IC_int=-(((long)adc.Imeas_c_offset)<<1)*_IQ16mpy(adc.Imeas_c_gain, inv_params._1_I_nom);
//	}
